...
class PatchEmbed2D(nn.Module):
    ...
    """
    def __init__(self, patch_size=2, in_chans=3, embed_dim=96, norm_layer=None, **kwargs):
        super().__init__()
...
class SS_Conv_SSM(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        res_out = self.resblock(input_left)
        fft_out = self.fftblock(input_left)
        conv_sum = res_out + fft_out
        conv_sum = conv_sum.permute(0, 2, 3, 1).contiguous()
        output = torch.cat((conv_sum, x), dim=-1)
